...
export const getFrom = (me: TimeRange): Time.Time => {
  return Time.fromTimestampOrThrow(me.from);
};

export const getTill = (me: TimeRange): Time.Time => {
  return Time.fromTimestampOrThrow(
    me.till <= MAXIMUM_FROM_TIMESTAMP 
      ? me.till
      : me.till - MAXIMUM_FROM_TIMESTAMP
  );
};

export const getFromTimestamp = (me: TimeRange): number => {
  return me.from;
};

export const getTillTimestamp = (me: TimeRange): number => {
  return me.till;
};

export const toString = (me: TimeRange): string => {
  const from = Time.toString(getFrom(me));
  const till = Time.toString(getTill(me));
  return `${from} .. ${till}`;
}