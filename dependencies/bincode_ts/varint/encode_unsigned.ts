import { SINGLE_BYTE_MAX, U128_BYTE, U16_BYTE, U16_MAX, U32_BYTE, U64_BYTE } from "./common.ts"
import { EncodeError } from "../"
import { IntegerEndianness, Write } from "../prelude.ts";

const Integer_toBigEndianBytes = (me: number): Uint8Array => {
  throw new Error("todo");
};
const Integer_toLittleEndianBytes = (me: number): Uint8Array => {
  throw new Error("todo");
};

export const encodeVariableLengthedU16 = <Writer>(
  number: number,
  write: Write<Writer>,
  writer: Writer,
  endianness: IntegerEndianness.IntegerEndianness,
): any => {
  if (number <= SINGLE_BYTE_MAX) {
    return write.writeByte(writer, number);
  }

  write.writeByte(writer, U16_BYTE);
  IntegerEndianness.match(endianness, {
    Big: () => write.write(writer, Integer_toBigEndianBytes(number)),
    Little: () => write.write(writer, Integer_toLittleEndianBytes(number)),
  });
};

export const encodeVariableLengthedU32 = <Writer>(
  number: number,
  write: Write<Writer>,
  writer: Writer,
  endian: IntegerEndianness.IntegerEndianness,
): any => {
  if (number <= SINGLE_BYTE_MAX) {
    return write.writeByte(writer, number);
  }
  ...
  if (number <= U16_MAX) {
    write.writeByte(writer, U16_BYTE);
    IntegerEndianness.match(endian, {
      Big: () => write.write(writer, Integer_toBigEndianBytes(number)),
      Little: () => write.write(writer, Integer_toLittleEndianBytes(number)),
    });
    return;
