...
export const enum BincodeErrorType {
  Unimplemented,
  OverflowLimit,
  InvalidLength,
  InvalidVariant,
...
export type BincodeError1 = {
  readonly type: BincodeErrorType,
};

export type PrimitiveTypeMarker = `u${IntBitSize}` 
  | `i${IntBitSize}` 
  | `f${FloatBitSize}` 
  | 'bool' | 'String'
...
export const Enum = <T extends {[variant: string]: EnumVariant }>(variants: T): Enum<T> => {
...
export const Struct = <T extends {[variant: string]: Type }>(fields: T): Struct<T> => {
...
export const decode = <T>(
  ...
  buffer: ArrayBuffer, 
  offset = 0, 
  config: BincodeConfig = BincodeConfig.STANDARD,
): {
  value: Value<T>
  offset: number
} => {
